...
class MatchSettings(BaseModel):
    ...
    high_confidence_threshold: float = Field(default=0.86, ge=0.5, le=1.0, description="Auto-accept threshold")
    medium_confidence_threshold: float = Field(default=0.72, ge=0.3, le=0.95, description="Review threshold")
    low_confidence_threshold: float = Field(default=0.55, ge=0.1, le=0.9, description="Low confidence cutoff")
    candidate_top_k: int = Field(default=4, ge=1, le=20, description="Number of candidates kept per segment")
    ...
    use_audio_rerank: bool = Field(default=False, description="Use background-audio similarity during visual rerank")
    use_multimodal_rerank: bool = Field(default=False, description="Enable optional multimodal rerank")
    global_backtrack_penalty: float = Field(default=1.2, ge=0.0, le=5.0, description="Penalty for time backtracking")
    duplicate_scene_penalty: float = Field(default=0.55, ge=0.0, le=3.0, description="Penalty for repeated overlapping scenes")
    use_dinov2_retrieval: bool = Field(default=True, description="Use DINOv2 + FAISS as the primary coarse retrieval engine")
    dino_model_name: str = Field(default="dinov2_vits14", description="Torch Hub DINOv2 model name")
    dino_batch_size: int = Field(default=24, ge=1, le=64, description="DINOv2 inference batch size")
    dino_top_k: int = Field(default=64, ge=4, le=128, description="Top-K FAISS neighbors per narration frame")
    dino_index_interval: float = Field(default=1.0, ge=0.5, le=10.0, description="Sampling interval in seconds for the DINO movie index")
